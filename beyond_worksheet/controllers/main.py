...
class OwnerSignature(http.Controller):
    ...
    def check_member(self, member_id):
        member = request.env['team.member'].sudo().search(
            [('member_id', '=', member_id)], limit=1)
        return {'exists': bool(member), 'member_id': member.id}

    @http.route('/worksheet/members/checkin/<int:user_id>/<int:worksheet_id>',
    ...
    @http.route('/my/task/<int:task_id>/signature/check', type='json',
                website=True)
    def task_signature_check(self, task_id, name=None, signature=None,
                             **kwargs):
    ...
    def show_question(self, worksheet, member, **kwargs):
        ...
        check_in = worksheet.worksheet_attendance_ids.filtered(
            lambda a: a.date.date() == fields.datetime.today().date() and
                      a.member_id.id == int(member_id) and a.type == 'check_in')
        if check_in:
            return request.render(
                'beyond_worksheet.portal_team_member_questions_done',
                {'worksheet': worksheet_id, 'member': member_id})
        questions = worksheet.member_question_ids.filtered(
            lambda q: q.date.date() == fields.datetime.today().date() and
                      q.member_id.id == int(member_id)).mapped('question_id')
        if questions and not check_in:
            questions = request.env['team.member.question'].sudo().search(
                [('id', 'not in', questions.ids)], limit=1)
        ...
        return request.render('beyond_worksheet.portal_team_member_question', {
            'question': questions[0], 'worksheet': worksheet_id,
            'member': member_id
        })

    @http.route(['/my/questions/answer'], type='http', auth="public",
                website=True)
    def submit_answer(self, **kwargs):
        ...
        return request.redirect('/my/questions/%s/%s' % (
        kwargs.get('worksheet_id'), kwargs.get('member_id')))

    @http.route('/team/member/checkout/<int:worksheet_id>/<int:member_id>',
    ...
    def member_checkout(self, worksheet_id, member_id, **kwargs):
        ...
        check_in = worksheet.worksheet_attendance_ids.filtered(
            lambda a: a.date.date() == fields.datetime.today().date()
                      and a.member_id.id == int(member_id) and a.type == 'check_in')
        check_out = worksheet.worksheet_attendance_ids.filtered(
            lambda a: a.date.date() == fields.datetime.today().date()
                      and a.member_id.id == int(member_id) and a.type == 'check_out')
