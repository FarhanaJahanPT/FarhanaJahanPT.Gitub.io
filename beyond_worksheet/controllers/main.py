...
class OwnerSignature(http.Controller):
    ...
    def member_signature_check(self, worksheet_id, survey_id, name=None, signature=None, **kwargs):
        ...
        # if not task.exists():
        #     return http.request.not_found()
        # survey.customer_name = name
        survey.signature = signature
        # task.date_worksheet_client_signature = datetime.now()
        return {
            'force_refresh': True,
            'redirect_url': f"/team/member/checkout/{worksheet_id}/{survey.team_member_id.id}"
        }
    ...
    # show installation question also
    ...
    def survey_submit(self, **post):
        ...
        if survey.is_from_worksheet:
            ...
            # request.render("beyond_worksheet.portal_team_member_signature",{'survey':survey_id,'user_input_id':latest_input.id})
    ...
    def worksheet_members_check_in(self, survey_id, worksheet_id, member_id, latitude, longitude, **kw):
        ...
        latest_input = survey.user_input_ids.sudo().sorted(key=lambda r: r.create_date, reverse=True)[:1]
        today = datetime.today().date()
        # checkin = request.env['worksheet.attendance'].sudo().search([
        #     ('member_id', '=', int(member_id)),
        #     ('type', '=', 'check_in'),
        #     ('create_date', '>=', str(today) + ' 00:00:00'),
        #     ('create_date', '<=', str(today) + ' 23:59:59')
        ...
        # print(existing_attendance, "existing record innnnnnnn")
        # if checkin:
        #     return request.render('beyond_worksheet.portal_team_member_questions_done',
